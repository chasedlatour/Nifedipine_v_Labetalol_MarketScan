...
%macro setup(sample, ProgramName, saveLog=N, serverSpace=local);
   ...
   /* DEFINE PROJECT SPECIFIC FILE PATHS, DEPENDENT ON SAMPLE */
      ...
      %IF &serverSpace = local %THEN %LET ProjDir = /local/projects/&db./&IRB./&proj.;
      	%ELSE %IF &serverSpace = nearline %THEN %LET ProjDir = /nearline/files/projects/&db./&IRB./&proj.;;
   ...
   /* DEFINE LIBNAMES */
      ...
      %END;
		%ELSE %IF &db = marketscan_preg %THEN %DO;
				libname raw ("/local/data/master/marketscanccae/pregnancy/&sample"
