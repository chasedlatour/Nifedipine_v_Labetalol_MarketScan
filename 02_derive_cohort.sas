...
%macro derive_initial_cohort(lmpindex, maxfill);
	...
	*******************/
	...
	set ana.preg_cohort_&lmpindex;
		...
		%*If someone has an outcome before their indexing prenatal encounter, then remove the pregnancy. This could occur because we used
	...
	proc sql;
		...
		having a.dt_gapreg > b.svcdate /*Only include those individuals whose fills occurred BEFORE the end of the pregnancy.*/
		;
		quit;
...
proc contents data=out.preg_cohort_63_dt_index_270;run;
