...
%implement_exclusions(inds=ana.preg_covar_63_dt_index_270_m21, outds=sens_lmpm21_cohort;)

*Get the distribution by treatment;
proc freq data=sens_lmpm21_cohort;
	table trt / missing;
run;

proc freq data=sens_lmpm21_cohort (where = (ga_at_index < 98));
	table trt / missing;
run;
