/********************************************************************************************************************************************
PROGRAM: 06_enrl_bounds_ptb_loss.sas
...
%setup(sample=full, programname=06_enrl_bounds_ptb_loss, savelog=Y);
...
set ana.primary_cohort;
	...
	*Determine which pregnancies were LTFU due to disenrollment <= 31 days after their preg outcome;
	days_disenroll = dt_disenroll_post_any - dt_gapreg; *Still use original pregnancy outcome date here;
	...
	***Assumption: Both nifedipine and labetalol initiators IMMEDIATELY experienced a loss upon censoring;
	
	*Event indicator;
	if preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_both = 'SAB';
		else preg_outcome_clean_both = preg_outcome_ptb;

	*New date indicator ;
	if preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_both = dt_gapreg_ptb; *-- The maximum of 23w gestation or their outcome date;
		else dt_gapreg_both = dt_gapreg_ptb;

	***Assumption: Neither nifedipine nor labetalol initiators expreienced the study outcome and survived the full follow-up;

	*Event indicator;
	if preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_none = 'TB';
		else preg_outcome_clean_none = preg_outcome_ptb;

	*New date indicator;
	if preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_none = dt_lmp + 258;
		else dt_gapreg_none = dt_gapreg_ptb;


	***Assumption: Nifedipine initiators IMMEDIATELY experienced a loss while labetalol initiators NEVER experienced the study outcome;

	*Event indicator;
	if trt = 1 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_nif = 'SAB';
		else if trt = 0 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_nif = 'TB';
		else preg_outcome_clean_nif = preg_outcome_ptb;

	*New event date;
	if trt = 1 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_nif = dt_gapreg_ptb;
		else if trt = 0 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_nif = dt_lmp + 258;
		else dt_gapreg_nif = dt_gapreg_ptb;


	***Assumption: Labetalol initiators IMMEDIATELY experienced a loss while labetalol initiators NEVER experienced the study outcome;

	*Event indicator;
	if trt = 1 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_lab = 'TB';
		else if trt = 0 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_lab = 'SAB';
		else preg_outcome_clean_lab = preg_outcome_ptb;

	*New event date;
	if trt = 1 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_lab = dt_lmp + 258;
		else if trt = 0 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_lab = dt_gapreg_ptb;
...
proc freq data=partial_bounds noprint;
	...
	table daysle31 / out=ana.partial_bounds_count_ptb;
run;
...
%macro partial_bounds_ptb(
	...
	%do n=1 %to &numassumptions;
		...
		%competing3risk_weights(
			...
			numiterations=1,
			initialseed=23244, 
			outds=ana.partialb_point_ptb2_&assumption,
			outds_dist=gadist_partialb_point_ptb2_&assumption,
			outds_ps = NA,
			outds_surv = NA
			);
		...
		%combine_point_estimates(
			gadist = ana.gadist_partialb_point_ptb2_&assumption,
			numgastrat = 2,
			est = ana.partialb_point_ptb2_&assumption._,
			outds = ana.partialb_point_ptb2_overall_&assumption
			);
		...
		%competing3risk_weights(
			...
			initialseed=23244, 
			outds=ana.partialb_boot_ptb2_&assumption,
			outds_dist=gadist_partialb_boot_ptb2_&assumption,
			outds_ps = NA,
			outds_surv = NA
			);
		...
		*Combine all the bootstrapped estimates;
		%combine_boot_estimates(
					inputEst= ana.partialb_boot_ptb2_&assumption, 
					inputDist= ana.gadist_partialb_boot_ptb2_&assumption,
					numStrata= 2, 
					output_stratified= ana.partialb_boot_ptb2_strat_&assumption,
					output_overall= ana.partialb_boot_ptb2_overall_&assumption);


		*Stratified estimates with confidence intervals.;
		%strat_estimates_w_CI(bootdsn=ana.partialb_boot_ptb2_&assumption._1, pointdsn=ana.partialb_point_ptb2_&assumption._1, output=ana.partialb_boot_ptb2_&assumption._1_ci);
		%strat_estimates_w_CI(bootdsn=ana.partialb_boot_ptb2_&assumption._2, pointdsn=ana.partialb_point_ptb2_&assumption._2, output=ana.partialb_boot_ptb2_&assumption._2_ci);

		*OVerall estimates with confidence interval;
		%overall_estimates_w_CI(stderrdsn=ana.partialb_boot_ptb2_overall_&assumption, pointdsn=ana.partialb_point_ptb2_overall_&assumption, output=ana.partialb_all_boot_ptb2_&assumption._ci);
...
%partial_bounds_ptb(numboot=1000); 




/****Assumption: Neither had an outcome;*/
%count_missing_zero(inds1=ana.partialb_boot_ptb2_none_1, inds2=ana.partialb_boot_ptb2_none_2);

***Both had an outcome;
%count_missing_zero(inds1=ana.partialb_boot_ptb2_both_1, inds2=ana.partialb_boot_ptb2_both_2);

/****Assumption: Nifedipine had an outcome;*/
%count_missing_zero(inds1=ana.partialb_boot_ptb2_nif_1, inds2=ana.partialb_boot_ptb2_nif_2);

/****Assumption: Labetalol had an outcome;*/
%count_missing_zero(inds1=ana.partialb_boot_ptb2_lab_1, inds2=ana.partialb_boot_ptb2_lab_2);
