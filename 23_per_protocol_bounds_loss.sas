/********************************************************************************************************************************************
PROGRAM: 23_per_protocol_bounds.sas
PROGRAMMER: Chase Latour
PURPOSE: To calculate bounds on the per-protocol effect for pregnancy loss. 
...
TABLE OF CONTENTS:
	...
	- 02 - CALCULATE THE BOUNDS FOR PREGNANCY LOSS
...
%setup(sample=full, programname=23_per_protocol_bounds_loss, savelog=Y);
...
For pregnancy loss, we assumed: 
1. Nifedipine initiators immediately experienced pregnancy loss upon deviation from the protocol, while labetalol initiators experienced 
a term birth (at 39 weeks’ gestation or immediately upon censoring if after 39 weeks), and  
2. Vice-versa. 
...
set per_protocol_analysis;
	...
	****Assumption: Nifedipine initiators IMMEDIATELY experienced pregnancy loss upon deviation from the protocol, while
	labetalol initiators experienced a term birth at 39 weeks gestation (or immediately upon censoring if after 39 weeks);

	*Event indicator;
	if trt=1 and gap = 1 then preg_outcome_nif = 'SAB';
		else if trt=0 and gap = 1 then preg_outcome_nif = 'LBS';
		else preg_outcome_nif = preg_outcome_clean;

	*New event date variable;
	if trt = 0 and gap=1 then dt_gapreg_nif = max(dt_lmp + 273, dt_gapreg);
		else dt_gapreg_nif = dt_gapreg;

	****Assumption: Labetalol initiators IMMEDIATELY experienced pregnancy loss upon deviation from the protocol, while
	nifedipine initiators experienced a term birth at 39 weeks gestation (or immediately upon censoring if after 39 weeks);

	*Event indicator;
	if trt = 0 and gap = 1 then preg_outcome_lab = 'SAB';
		else if trt=1 and gap = 1 then preg_outcome_lab = 'LBS';
		else preg_outcome_lab = preg_outcome_clean;

	*New event date variable;
	if trt = 1 and gap=1 then dt_gapreg_lab = max(dt_lmp + 273, dt_gapreg);
...
%macro perprotocol_bounds_loss(
	...
	%do n=1 %to &numassumptions;
		...
		%competing2risk_weights(
			...
			numiterations=1,
			initialseed=23244, 
			outds=ana.perpro_point_&assumption,
			outds_dist=gadist_perpro_point_&assumption,
			outds_ps = NA,
			outds_surv = NA
			);
		...
		%combine_point_estimates(
			gadist = ana.gadist_perpro_point_&assumption,
			numgastrat = 2,
			est = ana.perpro_point_&assumption._,
			outds = ana.perpro_point_overall_&assumption
			);
		...
		%competing2risk_weights(
			...
			initialseed=23244, 
			outds=ana.perpro_boot_&assumption,
			outds_dist=gadist_perpro_boot_&assumption,
			outds_ps = NA,
			outds_surv = NA
			);
		...
		*Combine all the bootstrapped estimates;
		%combine_boot_estimates(
					inputEst= ana.perpro_boot_&assumption, 
					inputDist= ana.gadist_perpro_boot_&assumption,
					numStrata= 2, 
					output_stratified= ana.perpro_boot_strat_&assumption,
					output_overall= ana.perpro_boot_overall_&assumption);

		*Stratified estimates with confidence intervals.;
		%strat_estimates_w_CI(bootdsn=ana.perpro_boot_&assumption._1, pointdsn=ana.perpro_point_&assumption._1, output=ana.perpro_boot_&assumption._1_ci);
		%strat_estimates_w_CI(bootdsn=ana.perpro_boot_&assumption._2, pointdsn=ana.perpro_point_&assumption._2, output=ana.perpro_boot_&assumption._2_ci);

		*OVerall estimates with confidence interval;
		%overall_estimates_w_CI(stderrdsn=ana.perpro_boot_overall_&assumption, pointdsn=ana.perpro_point_overall_&assumption, output=ana.perpro_overall_boot_&assumption._ci);
...
*options mprint;
%perprotocol_bounds_loss(numboot=1000);

/****Assumption: Nifedipine had an outcome;*/
%count_missing_zero(inds1=ana.perpro_boot_nif_1, inds2=ana.perpro_boot_nif_2);

/****Assumption: Labetalol had an outcome;*/
%count_missing_zero(inds1=ana.perpro_boot_lab_1, inds2=ana.perpro_boot_lab_2);
