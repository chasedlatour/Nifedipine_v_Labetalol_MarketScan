...
%setup(sample=full, programname=06_enrl_bounds_ptb, savelog=Y);
...
set ana.primary_cohort;
	...
	*Event indicator;
	if preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_both = 'PTB';
		else preg_outcome_clean_both = preg_outcome_ptb;

	*New date indicator ;
	if preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_both = max(dt_lmp + 161, dt_gapreg_ptb); *-- The maximum of 23w gestation or their outcome date;
	...
	*Event indicator;
	if trt = 1 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_nif = 'PTB';
	...
	*New event date;
	if trt = 1 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_nif = max(dt_lmp + 161, dt_gapreg_ptb);
	...
	*Event indicator;
	if trt = 1 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_lab = 'TB';
		else if trt = 0 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then preg_outcome_clean_lab = 'PTB';
	...
	*New event date;
	if trt = 1 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_lab = dt_lmp + 258;
		else if trt = 0 and preg_outcome_ptb = 'UNK' and daysle31 = 0 then dt_gapreg_lab = max(dt_lmp + 161, dt_gapreg_ptb);
...
proc freq data=partial_bounds;
	where preg_outcome_ptb = "UNK" and daysle31 = 0;
	table trt / missing;
run;
...
%macro partial_bounds_ptb(
	...
	%do n=1 %to &numassumptions;
		...
		%competing3risk_weights(
			...
			numiterations=1,
			initialseed=23244, 
			outds=ana.partialb_point_ptb_&assumption,
			outds_dist=gadist_partialb_point_ptb_&assumption,
			outds_ps = NA,
			outds_surv = NA
			);
		...
		%combine_point_estimates(
			gadist = ana.gadist_partialb_point_ptb_&assumption,
			numgastrat = 2,
			est = ana.partialb_point_ptb_&assumption._,
			outds = ana.partialb_point_ptb_overall_&assumption
			);
		...
		%competing3risk_weights(
			...
			initialseed=23244, 
			outds=ana.partialb_boot_ptb_&assumption,
			outds_dist=gadist_partialb_boot_ptb_&assumption,
			outds_ps = NA,
			outds_surv = NA
			);
		...
		*Combine all the bootstrapped estimates;
		%combine_boot_estimates(
					inputEst= ana.partialb_boot_ptb_&assumption, 
					inputDist= ana.gadist_partialb_boot_ptb_&assumption,
					numStrata= 2, 
					output_stratified= ana.partialb_boot_ptb_strat_&assumption,
					output_overall= ana.partialb_boot_ptb_overall_&assumption);


		*Stratified estimates with confidence intervals.;
		%strat_estimates_w_CI(bootdsn=ana.partialb_boot_ptb_&assumption._1, pointdsn=ana.partialb_point_ptb_&assumption._1, output=ana.partialb_boot_ptb_&assumption._1_ci);
		%strat_estimates_w_CI(bootdsn=ana.partialb_boot_ptb_&assumption._2, pointdsn=ana.partialb_point_ptb_&assumption._2, output=ana.partialb_boot_ptb_&assumption._2_ci);

		*OVerall estimates with confidence interval;
		%overall_estimates_w_CI(stderrdsn=ana.partialb_boot_ptb_overall_&assumption, pointdsn=ana.partialb_point_ptb_overall_&assumption, output=ana.partialb_all_boot_ptb_&assumption._ci);
...
%count_missing_zero(inds1=ana.partialb_boot_ptb_none_1, inds2=ana.partialb_boot_ptb_none_2);

***Both had an outcome;
%count_missing_zero(inds1=ana.partialb_boot_ptb_both_1, inds2=ana.partialb_boot_ptb_both_2);

/****Assumption: Nifedipine had an outcome;*/
%count_missing_zero(inds1=ana.partialb_boot_ptb_nif_1, inds2=ana.partialb_boot_ptb_nif_2);

/****Assumption: Labetalol had an outcome;*/
%count_missing_zero(inds1=ana.partialb_boot_ptb_lab_1, inds2=ana.partialb_boot_ptb_lab_2);
