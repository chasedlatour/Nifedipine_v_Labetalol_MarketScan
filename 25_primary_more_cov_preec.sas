...
%setup(sample=full, programname=25_primary_more_cov_preec, savelog=Y);
...
set ana.preeclampsia_outc;
	where region ne '5' and preeclampsia_pre = 0;
run;
...
%competing3risk_weights(
	...
	numiterations=1,
	initialseed=23244, 
	outds=ana.moreadj_point_pre,
	outds_dist=ga_dist_moreadj_point_pre,
	outds_ps = NA,
	outds_surv = NA
	);
...
%combine_point_estimates(
	gadist = ana.ga_dist_moreadj_point_pre,
	numgastrat = 2,
	est = ana.moreadj_point_pre_,
	outds = ana.moreadj_point_pre_overall
	);
...
%competing3risk_weights(
	...
	numiterations=1000,
	initialseed=23244, 
	outds=ana.moreadj_boot_pre,
	outds_dist=ga_dist_moreadj_boot_pre,
	outds_ps = ps_moreadj_boot_pre,
	outds_surv = NA
	);
...
%combine_boot_estimates(
			inputEst= ana.moreadj_boot_pre,
			inputDist= ana.ga_dist_moreadj_boot_pre,
			numStrata= 2, 
			output_stratified= ana.moreadj_pre_boot_strat,
			output_overall= ana.moreadj_pre_boot_overall);

*Stratified estimates with confidence intervals.;
%strat_estimates_w_CI(bootdsn=ana.moreadj_boot_pre_1, pointdsn=ana.moreadj_point_pre_1, output=ana.moreadj_boot_pre_1_ci);
%strat_estimates_w_CI(bootdsn=ana.moreadj_boot_pre_2, pointdsn=ana.moreadj_point_pre_2, output=ana.moreadj_boot_pre_2_ci);

*OVerall estimates with confidence interval;
%overall_estimates_w_CI(stderrdsn=ana.moreadj_pre_boot_overall, pointdsn=ana.moreadj_point_pre_overall, output=ana.moreadj_overall_pre_boot_ci);

	
%count_missing_zero(inds1=ana.moreadj_boot_pre_1, inds2=ana.moreadj_boot_pre_2);
