...
%setup_mgp(sample=random1pct, programname=02_derive_cohort, savelog=N);
...
%macro derive_initial_cohort(lmpindex, maxfill);
	...
	*/
	...
	proc sql noprint;
	    ...
	    from ana.preg_cohort_&lmpindex;
		quit;
	...
	*******************/
	...
	set ana.preg_cohort_&lmpindex;
		...
		if dt_indexprenatal > dt_gapreg then delete; /*MP 2/3/25: edited dt_indexprenatal (was dt_idexprenatal)*/
	...
	proc sql;
		create table temp.pregnancies_with_index_&lmpindex as
...
%macro identify_new_users(lmpindex, lookbackdt, lookbackdays);
	...
	*/
	...
	proc sql;
		create table temp.preg_newusers_&lmpindex._&lookbackdt._&lookbackdays as
		select distinct a.*
...
%macro identify_dx_covariates(REF=, PREG_INPT=, OUTPUT=);
	...
	data temp.diagnoses_covariate_&lmpindex; set _diagnoses_covariate; run;
	...
	*********************;
	...
	proc sql;
		create table _diagnoses3 as
		select distinct enrolid, idxpren, diagnosis, location, timing, count(distinct cats(svcdate, count)) as numdx /*These codes need to be on different dates at least 30 days apart*/
		from _diagnoses_30d
		group by enrolid, idxpren, diagnosis, location, timing
		;
		quit;
...
/*	run;*/
	...
	*************/
	data temp.preg_incl_excl_&lmpindex._&lookbackdt._&lookbackdays;
	set _pregnancies6;
	run;
...
%macro identify_diabetes_dx(INPUT_DATA, OUTPUT_DATA, DIAGNOSES_DATA=_diagnoses2);
	...
	*/
	...
	data temp.diabetes_diagnoses_&lmpindex;
	...
	*Add on the additional information that we need for diabetes:
	...
	proc sql;
		...
		select enrolid, idxpren, location, count(distinct svcdate) as numdx
		from (select * from _diabetes_diagnoses2 where dt_lookback <= svcdate <= min(dt_lmp+90, dt_gapreg)) as a
		group by enrolid, idxpren, location
		;
	...
	*Add on the additional pregnancy information that we need for HTN:
	...
	%do d=1 %to &numlmp;
		...
		proc sql;
			...
			from _htn_diagnoses2 where dt_lmp + &&loop&d <= svcdate
			group by enrolid, idxpren, diagnosis, location, timing
			;
			quit;
		...
		proc sql;
			...
			select enrolid, idxpren, diagnosis, location, timing,
				count(distinct svcdate) as lmp&&loop&d
			from _htn_diagnoses2 where dxloc = "InptAdm" and dxnum <= 2 and dt_lmp + &&loop&d <= svcdate
			group by enrolid, idxpren, diagnosis, location, timing
			;
			quit;
...
%macro identify_rx_covariates(INPUT_DATA, OUTPUT_DATA, REF_DATA);
	...
	data temp.cov_meds_&lmpindex; set _cov_meds; run;
...
%macro get_healthcare_use(INPUT_DATA, OUTPUT_DATA);
	...
	proc sql;
		...
		select distinct a.*, sum(b.pren_outpatient)as num_OutptPNC, sum(b.pren_inpatient) as num_InptPNC,
			count(distinct c.enc_date) as num_SpecificGA_preidx, 
			count(distinct d.enc_date) as num_SpecificGA_preg
		from &INPUT_DATA as a
		...
		left join (select distinct * from temp.gestagepren (where = (code_hierarchy = "Specific gestational age"))) as d
		on a.patient_deid=d.patient_deid and a.dt_indexprenatal <= d.enc_date <= a.dt_gapreg + 7
		group by a.patient_deid, a.idxpren
		;
		quit;

	%*Second, identify the number of distinct inpatient admission dates prior to the index date;
...
%macro get_allcovariates(LMPINDEX, LOOKBACKDT, LOOKBACKDAYS);
	...
	**********;
	...
	data temp.preg_covar_&lmpindex._&lookbackdt._&lookbackdays;
...
%macro identfy_postid_antihypertensives(LMPINDEX, LOOKBACKDT, LOOKBACKDAYS);
	...
	*/
	...
	proc sql;
		create table out.preg_cohort_&lmpindex._&lookbackdt._&lookbackdays as
		/*CDL: 1.28.2025 - added distinct*/
