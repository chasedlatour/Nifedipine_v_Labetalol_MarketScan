/********************************************************************************************************************************************
PROGRAM: 05_full_bounds_loss.sas
...
estimat for pregnancy loss, under different assumptions about pregnancies with unobserved outcomes.
...
set ana.primary_cohort;
	...
	*Assume that none experienced the study outcome;
	if preg_outcome_clean = 'UNK' then none_outc = 0;
		else if preg_outcome_clean in ('SAB' 'UAB' 'SB' 'MLS') then none_outc = 1;
		else none_outc = 0;

	*Assume that both experienced the study outcome;
	if preg_outcome_clean = 'UNK' then both_outc = 1;
		else if preg_outcome_clean in ('SAB' 'UAB' 'SB' 'MLS') then both_outc = 1;
		else both_outc = 0;

	*Assume that nifedipine only experienced the study outcome;
	if preg_outcome_clean = 'UNK' and trt=1 then nif_outc = 1;
		else if preg_outcome_clean = 'UNK' and trt = 0 then nif_outc = 0;
		else if preg_outcome_clean in ('SAB' 'UAB' 'SB' 'MLS') then nif_outc = 1;
		else nif_outc = 0;

	*Assume that labetalol only experienced the study outcome;
	if preg_outcome_clean = 'UNK' and trt=1 then lab_outc = 0;
		else if preg_outcome_clean = 'UNK' and trt = 0 then lab_outc = 1;
		else if preg_outcome_clean in ('SAB' 'UAB' 'SB' 'MLS') then lab_outc = 1;
		else lab_outc = 0;
