...
of treatment and censoring weights, in the setting where there are 2 competing events that should be modeled separately.
...
- CR2 = value(s) of OUTCOMEVAR that represent competing risk event 2 - POSITIVE COMPETING EVENT (e.g., live birth)
...
%macro competing2risk_weights(
	...
	* 1a: create analytic variables for time to event and outcome;
	...
	set &inds;
		...
		if event in (1, 2, 3) then combined = 1;
			else combined = 0;
	run;
		...
		%DO g = 1 %TO &numGA;
			...
			*/ 
			...
			%IF &smr = 1 %THEN %DO;
				...
				%IF &boot = 0 and &outds_ps ne NA %THEN %DO;
			 	...
			 	%*Output the PS and SMR weight information for subsequent evaluation;
		      	data __anacohort&g; 
					...
					by _id;  
		         	...
		         	keep _id &trtvar expwgt event outcome comprisk1 comprisk2 combined days event &dovars;
		      	run;
			...
			%ELSE %DO;
				...
				%*Now, give evveryone an exposure weight of 1 - uninformative;
				data __anacohort&g;
				set _anacohort&g;
					expwgt=1;
					keep _id &trtvar expwgt event outcome comprisk1 comprisk2 combined days event &dovars;
			...
			%*Get maximum follow-up (in weeks) over the included pregnancies;
			proc sql noprint;
				select max(days)
				into :max_follow
				from __anacohort&g;
			...
			data ___anacohort&g; 
			set __anacohort&g(rename=(days=days1 outcome=outcome1 comprisk1=comprisk11 comprisk2=comprisk21 combined=combined1));
				...
				do j=1 to cat;
					...
					if in < days1 and days1 <= j*7 then do;
						...
						days = j*7;
						drop = 0;
						outcome=0; comprisk1=0; comprisk2=0; combined=0;
					end;
					output;

				end;

	         	keep _id in days outcome comprisk1 comprisk2 combined drop expwgt &trtvar &dovars;
	      	run;
...
/*				%END;*/
				wgt = expwgt * dowgt;
	         	keep _id &trtvar in days outcome comprisk1 comprisk2 combined wgt dowgt;
	     	run;
...
/* 	      	%*AJ code from Steve Cole, Cannot accomodate IPCW; */
/* 	      	proc phreg data=__anacohort&g noprint; */
/* 	      		strata &trtvar;  */
/* 	      		model days*event(0) = /eventcode = 1; */
/* 	      		baseline out=cif cif=_all_; */
/* 	      	run; */
/* 	      	proc freq data=__anacohort&g; */
/* 	      		table trt; */
/* 	      	run; */
	        ...
	        merge _combined _outcome _comprisk1 _comprisk2; 
				by &trtvar days;
	        	retain cum_outcome 0 cum_comprisk1 0 cum_comprisk2 0 olds_combined olds_outcome olds_comprisk1 olds_comprisk2 rate;
				if first.&trtvar then rate = .; 
	         	if days=0 then do; 
	            	olds_combined=1; olds_outcome=1; olds_comprisk1=1; olds_comprisk2 = 1;
	            	outcome=0; comprisk1=0; comprisk2=0;
					cum_outcome=0; cum_comprisk1=0; cum_comprisk2=0;
					e_outcome=0; e_comprisk1=0; e_comprisk2=0;
	         	end;
...
/* 					output; */
				...
				if s_comprisk2 ne . then do; 
	         	...
	         	combined=cum_outcome+cum_comprisk1+cum_comprisk2;
